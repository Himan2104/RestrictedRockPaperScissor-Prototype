...
void Debug::log()
{
    std::cout << Utility::getCurrentTime() << " INFO : " << "Empty Log function called! If enabled, it won't be logged in the logfile." << std::endl;
}
void Debug::log(std::string message, MessageType m_type)
{
    ...
    if(_clf) *_logfile << Utility::getCurrentTime() << type << message << std::endl;
}
